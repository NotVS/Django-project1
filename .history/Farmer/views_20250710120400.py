...
def registration(request):
    ...
    if request.method == 'POST':
        ...
        verification_url = request.build_absolute_uri(
            reverse('verification2', kwargs={'farmer_id': new_farmer.id}) # Ensure you have the correct URL name
        )
        
        
        send_mail(subject = 'Verification Email',
                  message = verification_url,
                  from_email = 'notvsmobile@gmail.com',
...
def crop_chart(request):
    ...
    if not farmer_id:
       return HttpResponse("Need to be logged in")
...
def verification2(request, farmer_id):
    try:
        farmer = Farmer.objects.get(id = farmer_id)
        farmer.verified = True
        farmer.save()
        return redirect(f"/welcome/{farmer.id}")
    except Farmer.DoesNotExist:
        return HttpResponse("Farmer not found")
    
    
def welcome(request, farmer_id):
    template = loader.get_template("welcome.html")
    farmer = Farmer.objects.get(id = farmer_id)
    
    return HttpResponse(template.render({'farmer_id' : farmer_id}, request))